...
void Builder::stopMotorDriver(const QString pageName, Page* page)
{
    QPushButton* rightAlignedButton = new QPushButton(page);

    rightAlignedButton->setObjectName(pageName + "_rightButton");
    guiWidgets[rightAlignedButton->objectName()] = rightAlignedButton;

    rightAlignedButton->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    rightAlignedButton->setStyleSheet(
        "QPushButton {"
    ...
    // Dikey yüksekliği 200 piksel olarak ayarla
    rightAlignedButton->setFixedWidth(100);
    rightAlignedButton->setFixedHeight(100);
    rightAlignedButton->setIcon(QIcon(":/icons/icons/stop.png"));
    rightAlignedButton->setIconSize(QSize(100, 100));
    connect(rightAlignedButton, &QAbstractButton::clicked, [rightAlignedButton]() {
        // Animasyonu oluştur
        QPropertyAnimation* animation = new QPropertyAnimation(rightAlignedButton, "iconSize");
        animation->setDuration(200);  // 200 ms süreyle animasyon
        animation->setStartValue(QSize(100, 100));  // Başlangıç boyutu
        animation->setKeyValueAt(0.5, QSize(120, 120));  // Ortada büyüt
        animation->setEndValue(QSize(100, 100));  // Eski boyuta döndür
        animation->start(QAbstractAnimation::DeleteWhenStopped);
    });

    // page->layout()->addWidget(rightAlignedButton);
}
